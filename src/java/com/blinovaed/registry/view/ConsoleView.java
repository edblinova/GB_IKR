package com.blinovaed.registry.view;

import com.blinovaed.registry.controller.KennelAccounting;
...
import com.blinovaed.registry.util.Counter;
...
public class ConsoleView implements View {
    ...
    public boolean showAddAnimalDialog() {
        String infoMessage = "Введите параметры животного в виде строки: \"имя день_рождения род_животного\"\n" +
                "день_рождения имеет вид: dd-mm-yyyy (05-04-2024): \n" +
    ...
    public boolean showAddSkillDialog(AbstractAnimal animal) {
        ...
        if (skillData.length == 1) {
            animal.getAnimalGenius().learnSkill(new Skill(skillData[0]));
        } else if (skillData.length == 2) {
            animal.getAnimalGenius().learnSkill(new Skill(skillData[0], skillData[1]));
        } else {
     ...
     *                  дата_рождения - имеет фоормат dd-MM-yyyy (05-04-2024)
    ...
    private AnimalData parceAnimalData(String inputData) {
        ...
        LocalDate birth_date = LocalDate.parse(input[1], formatter);
        AnimalGenius genius = AnimalGenius.valueOf(input[2].toUpperCase());
        return new AnimalData(animalName, birth_date, genius);
    }
    ...
    private void printAnimalInfo(AbstractAnimal animal) {
        ...
        System.out.printf("Умения: %s\n", animal.getAnimalGenius().getAnimalSkills());
        printLineWithSymbol("~", SIZE_LINE);
    }
}
